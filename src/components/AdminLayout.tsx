...
export default function AdminLayout() {
  ...
  const loadNotifsAndCounts = useCallback(async () => {
    const [{ data: nData }, { count }] = await Promise.all([
      supabase
            ...
            return (
              ...
              >
                ...
                {isVerifications && pendingVerifsCount > 0 && (
                  <span className="px-2 py-0.5 text-[11px] font-black bg-red-500 text-white rounded-full">
                    {pendingVerifsCount}
                  </span>
                )}
              </NavLink>
